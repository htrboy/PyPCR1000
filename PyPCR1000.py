...
class Application(tkinter.Tk):
	...
	def Power(self):			# The power button was pressed
		if DEBUG:
			print("power = " + str(self.radio.power))
		if bool(self.radio.power == 1):				# Radio is on
			self.radio.RadioPower(0)
		else:
		...
		if p.is_open:
			...
			if text:
				if LOGGER:
					LOGGER.write(text)
				if DEBUG:
					print("ReadSerial Text: " + str(text))
...
class PCR1000:
			...
			while n:
				try:
					...
					if LOGGER:
						LOGGER.write(text)
					if DEBUG:
						print("PollSerial text: " + str(text))
	...
	def RadioPower(self, turn_on):
		if DEBUG:
			print("RadioPower called with " + str(turn_on))
			print("Serial port status: " + str(self.serialport.is_open))
		tries = 5
		if turn_on:
			if DEBUG:
				print("turning on...")
			...
			if not self.serialport.is_open:
				...
				if not self.serialport.is_open:
					if DEBUG and LOGGER:
						...
						print("Serial port %s could not be opened. /n" % self.serialport.port())
						LOGGER.write(t)
					return
				self.parse_n = 0

			if DEBUG:
				print("checking LOGGER instance...")
			...
			if not self.serialport.getCTS():
				if DEBUG:
					print("CTS: " + str(self.serialport.getCTS()))								
			# PCR1000 returns CTS status for power switch on/off
				if DEBUG and LOGGER:
					...
					LOGGER.write(t)
				if DEBUG:
					print("returning from check CTS...")
			...
			if self.power == 1:
				if self.serialport.getBaudrate() != 38400:
					...
					time.sleep(0.1)
					if DEBUG:
						print("setting baudrate to 38400")
					self.serialport.setBaudrate(38400)
...
class DialogSerial(tkinter.Toplevel):
	def __init__(self, serialport, master, cnf={}, **kw):
		...
		#self.varPort = tkinter.IntVar() // change to stringtype (Python3)
		self.varPort = tkinter.StringVar()
	...
	def Open(self):
		...
		if self.serialport.isOpen(self):
		#if self.varSerOpen:
			self.serialport.close(self)
		else:
			self.SetPort()
			try:
				self.serialport.open(self)
			except:
				FormatTb()
